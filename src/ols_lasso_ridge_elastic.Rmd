...
ols_preds <- predict(ols_model, newdata = df_cleaned)
ols_results <- data.frame(
  Model = "OLS (Baseline)",
  RMSE = round(rmse(Y, ols_preds), 5),
  MAPE = round(mape(Y, ols_preds) * 100, 5),
  R2 = round(rsq(Y, ols_preds), 5)
)

# ---- Init results ----
results_oos <- ols_results  # start with OLS row
```
...
results_oos <- rbind(
  results_oos,
  data.frame(Model = paste0("Elastic Net (α=", best_alpha, ")"),
...
-- run the models again but without outliers -- 
remove outliers
```{r}
# Calculate IQR-based thresholds
Q1 <- quantile(df_cleaned$resale_price, 0.25)
Q3 <- quantile(df_cleaned$resale_price, 0.75)
IQR_val <- Q3 - Q1
...
df_no_outliers <- df_cleaned[df_cleaned$resale_price >= lower_bound & df_cleaned$resale_price <= upper_bound, ]

# View number of rows removed
cat("Removed", nrow(df_cleaned) - nrow(df_no_outliers), "rows as outliers.\n")
...
# ---- Prepare data ----
set.seed(123)
X <- model.matrix(resale_price ~ . -1, data = df_no_outliers)
Y <- df_no_outliers$resale_price
n <- nrow(X)
train_idx <- sample(1:n, size = 0.8 * n)
X_train <- X[train_idx, ]
Y_train <- Y[train_idx]
...
```{r}
# ---- OLS Baseline ----
ols_model <- lm(resale_price ~ ., data = df_no_outliers)
ols_preds <- predict(ols_model, newdata = df_no_outliers)
ols_results <- data.frame(
  Model = "OLS (Baseline)",
  RMSE = round(rmse(Y, ols_preds), 5),
  MAPE = round(mape(Y, ols_preds) * 100, 5),
